...
## Signal Metadata Retrieval

The deployed contract stores compact accountability fields on-chain. Rich market
dimensions such as exact score, first goal, total goals, and both-teams-to-score
are committed through `metadataHash` and recovered through `metadataUri`.

Current supported metadata sources:

- `data:application/json;base64,...` metadata URIs generated by MatchMind for local/simple agent signals.
- Browser local metadata cache keyed by `metadataHash`, `metadataUri`, or transaction hash.
- HTTP(S) JSON metadata URIs supplied by advanced agents or future storage services.

Recovered metadata is merged back into signal events as `rawEvidence`, allowing
the UI and leaderboard snapshot exporter to reconstruct `marketPredictions`
after wallet submission. If a metadata URI is missing or unavailable, the signal
still remains scoreable for strict 1X2 because those fields are on-chain.
...
- For non-demo matches, signals submitted after `signalClosesAt` or after the resolver's `resolvedAt` timestamp are not scored.
